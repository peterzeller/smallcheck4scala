...
trait Property {
   ...
   */
  def map(f: TestCase => TestCase): Property = new Property {
    def apply(d: Int): Seq[TestCase] =
   ...
   */
  def flatMap(f: TestCase => Property): Property = new Property {
    def apply(d: Int): Seq[TestCase] =
  ...
  class WithFilter(p: TestCase => Boolean) {
    ...
    /** map over a filter */
    def map[B](f: TestCase => TestCase): Property = new Property {
      def apply(d: Int): Seq[TestCase] = {
    ...
    def flatMap[B](f: TestCase => Property): Property = new Property {
      def apply(d: Int): Seq[TestCase] = {
        val buffer = new ListBuffer[TestCase]
        for (tc <- Property.this.apply(d))
   ...
   */
  def combine(that: Property)(f: (TestCase, TestCase) => TestCase): Property =
   ...
   */
  def &&(p: Property): Property = combine(p)(_ && _)
...
object Property {
  ...
  case class TestCase(result: TestResult, args: Seq[String]) {
    ...
    def &&(that: TestCase): TestCase = (this.result, that.result) match {
      case (Fail, _) => this
      case (_, Fail) => that
  ...
  private def existance[A,P](unique: Boolean, s: Series[A])
    ...
    def apply(d: Int): Seq[TestCase] = {
      val witnesses = for {
    	  				...
    	  				p = f(a)
    	  				t = test(p)
    	  				if t(d).forall(tc => tc.resultIsOk)
                      } yield a.toString
